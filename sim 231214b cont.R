...
yg.tru = beta[1]+beta[2]*xg # pnorm((beta[1]+beta[2]*xg)/sqrt(1+beta[3]^2+beta[4]^2))
n = 30 # 120 # 60 # 
nr = 1000
nm = 3
one = rep(1, n)
...
for (r in 1:nr) {
	...
	mu.y = cbind(one,x,w,u)%*%beta
	y = mu.y+rnorm(n) # rbinom(n, 1, mu.y) # 
	all.est[r,1,] = est.ua(x, y, xg=xg)
	all.est[r,2,] = est.gc(w, x, y, xg=xg)
	all.est[r,3,] = est.iv(z, x, y, xg=xg)
}
...
file.out = paste("sim rst 231214b cont n=", n, ".csv", sep="")
