...
for (r in 1:nr) {
	...
	all.est[r,1,] = est.ua(x, y, family = binomial(link="logit"), xg=xg)
	all.est[r,2,] = est.gc(w, x, y, family = binomial(link="logit"), xg=xg)
	all.est[r,3,] = est.iv(z, x, y, family = binomial(link="logit"), xg=xg)
}
...
file.out = paste("sim rst 231214c bin logit n=", n, ".csv", sep="")
